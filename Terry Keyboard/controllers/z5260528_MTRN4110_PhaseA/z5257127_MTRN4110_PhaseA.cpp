...
auto goForwards(webots::Motor &leftMotor, webots::Motor &rightMotor) {
    ...
    double leftPos = leftMotor->getTargetPosition();
    double rightPos = rightMotor->getTargetPosition();
    // Get target coordinates
    leftPos = leftPos + 2.62586*M_PI;
    rightPos = rightPos + 2.62586*M_PI;
    // Move the robot to that location
...
void writeToFile(int step, int row, int col,char heading, bool left, bool front, bool right ) {
  ...
  std::cout<< "[z5257127_MTRN4110_PhaseA] " << "Step: " << std::setw(3) << std::setfill('0') << step <<","<< " Row: " << row << ","<< " Column: "<< col << "," << " Heading: "
...
int main() {
    ...
    // TERRY CODE HERE: KEYBOARD INITIALIZATION
    // Initialize the keyboard
    std::unique_ptr<webots::Keyboard> keyboard{robot.getKeyboard()};
    keyboard->enable(robot.step(TIME_STEP));
...
//  int Bearing = 2;
   ...
   //Open command file and save to string variable
    ...
    // TERRY CODE HERE: VARIABLE INITIALIZATION
    int key {0}; // Current key pressed
    int prevKey {0}; // Previous key pressed


    while (robot.step(TIME_STEP) != -1) {
        // TERRY CODE HERE: MANUAL OPERATION
        prevKey = key;
        key = keyboard->getKey()
        if (prevKey != key && key != -1) {
            switch(key) {
                // Forwards movement
                case 'W':
                    goForwards(leftMotor, rightMotor)
                    break;
                // Turn left
                case 'A':
                // Turn right
                ...
                // Pathfinding (automatic function) (current iteration does not account for changed robot position)
                case 'P':
                    // STEVEN'S CODE
